...
// Rate limiting store (resets on server restart)
// For production scaling, replace with Redis
...
5. Always end with a clear next action — either home management steps or "go to hospital now".
6. Write at a Grade 6 reading level. No Latin medical terms. Say "broken bone" not "fracture". Say "high blood pressure" not "hypertension".
7. Be calm and reassuring. The person reading this may be frightened.
8. Structure your response clearly with these sections when giving full guidance:
...
DISCLAIMER: Always end every full response with exactly this line:
"⚕️ This guidance does not replace a doctor. Seek professional medical help as soon as possible."`
}

function getConditionSystemPrompt(conditionName, conditionCategory) {
...
"⚕️ Guidance only. Seek professional medical help as soon as possible."`
}
...
export async function POST(request) {
  try {
    ...
    const mode = body.mode || 'triage' // 'triage' or 'condition'
    const rawQuestion = body.question || body.message || ''
    const conditionName = body.conditionName || ''
    const conditionCategory = body.conditionCategory || ''
    ...
    const systemPrompt = mode === 'condition'
      ? getConditionSystemPrompt(conditionName, conditionCategory)
    ...
    const aiText = response.content[0]?.text || 'I was unable to generate a response. Please try again.'
